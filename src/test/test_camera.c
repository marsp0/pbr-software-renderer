...
static void assert_frustum(float pitch, float yaw, vec4_t view_dir)
{
    ...
    ASSERT_VECTOR4(camera->n_plane.p, expected_n_p);
    ASSERT_VECTOR4(camera->n_plane.n, expected_n_n);

    ASSERT_VECTOR4(camera->f_plane.p, expected_f_p);
    ASSERT_VECTOR4(camera->f_plane.n, expected_f_n);

    ASSERT_VECTOR4(camera->r_plane.p, expected_r_p);
    ASSERT_VECTOR4(camera->r_plane.n, expected_r_n);

    ASSERT_VECTOR4(camera->l_plane.p, expected_l_p);
    ASSERT_VECTOR4(camera->l_plane.n, expected_l_n);

    ASSERT_VECTOR4(camera->t_plane.p, expected_t_p);
    ASSERT_VECTOR4(camera->t_plane.n, expected_t_n);

    ASSERT_VECTOR4(camera->b_plane.p, expected_b_p);
    ASSERT_VECTOR4(camera->b_plane.n, expected_b_n);
...
static void test_view_matrix_rotated_90deg_around_y()
{
    ...
    ASSERT_VECTOR4(position, camera->position);
    ASSERT_VECTOR4(forward, camera->forward);
...
static void test_proj_matrix()
{
    ...
    ASSERT_VECTOR4(position, vec4_new(0.f, 0.f, 1.f));

    // far plane point
    position = vec4_new(0.f, 0.f, -20.f);
    position = mat_mul_vec(actual, position);
    position = vec4_scale(position, 1.f/position.w);
    ASSERT_VECTOR4(position, vec4_new(0.f, 0.f, 0.f));
...
static void test_camera_visibility_check_when_mesh_is_visible()
{
    ...
    sphere_t sphere = { .c = vec4_new(0.f, 0.f, 5.f),
                        .r = 1.f };
